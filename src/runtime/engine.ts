...
export function buildSystemPrompt(options: ChatOptions): string {
  let prompt =
    ...
    "When the user provides a working directory or file path (e.g. \"Thư mục/File làm việc: 📁 /path/to/folder\" or relative/absolute file paths), immediately use file_read, glob, or grep to inspect it. If access is denied with PERMISSION_REQUEST, ask the user to approve that exact folder; never ask them to copy it into a workspace. Do NOT output generic responses when a file or directory path is given.\n\n" +
