 ...
 * scheduler ticks once a minute and asks `isDue` whether a task should run now,
 * given when it last ran. Pure and self-contained so it can be unit-tested
 * directly (see scripts/scheduler-check.mjs).
 *
 * Ported verbatim from the webview implementation: idea.md puts scheduling in
 * the Host Process, so this now lives with the runner and keeps firing while
 ...
 * Recognises: hourly, daily "at HH:MM", weekdays, a named weekday, and monthly
 * ("1st of each month"). Unknown text is treated as daily.
 */
